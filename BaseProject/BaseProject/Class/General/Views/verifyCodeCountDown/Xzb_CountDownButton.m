...
//    NSLog(@"_index = %ld",(long)_index);
    NSString *again_str = [NSString stringWithFormat:@"(%ld)秒后重发",(long)_index];
    ...
    if (_index <= 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self setTitle:[NSString stringWithFormat:@"(%ld)秒后重发",(long)self->_index] forState:UIControlStateNormal];
        });
